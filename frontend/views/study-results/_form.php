...
<div class="study-results-form">
    ...
    <div class="bg-surface-container-lowest p-10 rounded-xl shadow-sm space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <?= $form->field($model, 'publisher', FormUi::fieldConfig()['base'])->dropDownList($publishers, array_merge(FormUi::inputOptions()['select'], ['prompt' => 'Select Publisher'])) ?>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-10" id="other_publisher_name"
            style="<?= $model->publisher === 'other' ? '' : 'display:none' ?>">
            <?= $form->field($model, 'other_publisher_name', FormUi::fieldConfig()['base'])->textInput(array_merge(FormUi::inputOptions()['text'], ['maxlength' => true])) ?>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <?= $form->field($model, 'trial_id', FormUi::fieldConfig()['base'])->hiddenInput(array_merge(FormUi::inputOptions()['text'], ['readonly' => true]))->label(false) ?>
        </div>
...
$script = <<<JS
    function toggleOtherInstitution() {
        const institution = $('#studyresults-publisher').val();

        if (institution === 'other') {
            $('#other_publisher_name').slideDown();
        } else {
            $('#other_publisher_name').slideUp();
            $('#study-results-other_publisher_name').val('');
        }
    }

    toggleOtherInstitution();

    $('#studyresults-publisher').on('change', function () {
        toggleOtherInstitution();
    });
JS;
